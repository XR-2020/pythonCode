...
if __name__ == '__main__':
    ...
    for i in range(len(y_values)):
        plt.text(x_values[i],y_values[i],'%.2f'%(ioulist[i]),ha = 'center',va='bottom')
    # plt.show()

    # iousum = 0
    # num = 0
    # for num in range(0, 16):
    #     iousum = iousum + ioulist[num]
    # print("dicesum:", iousum, "num:", num + 1)
    # avg = iousum / 16
    # print("avg:", avg)
    # c = max(x_values)+1
    # d = (max(y_values)+min(y_values))/2
    # plt.text(c, d, 'iousum=%.2f' % iousum, ha='left', va='center', fontsize=15)
