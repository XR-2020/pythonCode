...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=768, norm_layer=None):
        ...
        self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=patch_size, stride=patch_size)#把整个大图像切成一个个小patch
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)#把大图像拉成由多个小patch组成的小图像
        x = self.norm(x)#在通道上归一化
        return x
